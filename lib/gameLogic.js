...
// --- Cultures & saisons ---
// Calendrier synchronisé sur l'horloge réelle : la saison est la même pour
// tout le monde en même temps (l'État décrète la saison), pas un minuteur
// propre à chaque partie.
export const SEASONS = ['printemps', 'ete', 'automne', 'hiver'];
export const SEASON_LABELS = { printemps: 'Printemps', ete: 'Été', automne: 'Automne', hiver: 'Hiver' };
export const SEASON_LENGTH_DAYS = 3;
export const CYCLE_LENGTH_DAYS = SEASON_LENGTH_DAYS * SEASONS.length; // 12
const DAY_MS = 86400000;

// growMult : multiplicateur sur le temps de pousse de base (>1 = plus lent).
// yieldMult : multiplicateur sur le rendement par récolte.
// price : prix de vente unitaire en pleine saison.
export const CROPS = {
  ble: { name: 'Blé', season: 'printemps', seedCost: 3, growMult: 1.0, yieldMult: 1.0, price: 5, emoji: '🌾' },
  orge: { name: 'Orge', season: 'ete', seedCost: 3, growMult: 0.85, yieldMult: 1.0, price: 5, emoji: '🌿' },
  pdt: { name: 'Pomme de terre', season: 'automne', seedCost: 3, growMult: 1.3, yieldMult: 1.6, price: 5, emoji: '🥔' },
  seigle: { name: 'Seigle', season: 'hiver', seedCost: 3, growMult: 1.15, yieldMult: 1.1, price: 5, emoji: '🌾' },
};
export const CROP_KEYS = Object.keys(CROPS);

// Pénalités de hors-saison : la pousse est plus lente ET le prix de vente
// est réduit — pas d'amende directe, la rotation des cultures se justifie
// d'elle-même par la perte de rendement économique.
export const OFF_SEASON_GROWTH_PENALTY = 1.6; // ×1.6 le temps de pousse
export const OFF_SEASON_PRICE_PENALTY = 0.5; // moitié prix

export function currentSeason(now = Date.now()) {
  const dayIndex = Math.floor(now / DAY_MS);
  const cycleDay = ((dayIndex % CYCLE_LENGTH_DAYS) + CYCLE_LENGTH_DAYS) % CYCLE_LENGTH_DAYS;
  return SEASONS[Math.floor(cycleDay / SEASON_LENGTH_DAYS)];
}

// 0..1 : position dans la saison en cours, pour une barre de progression.
export function seasonProgress(now = Date.now()) {
  const dayIndex = Math.floor(now / DAY_MS);
  const cycleDay = ((dayIndex % CYCLE_LENGTH_DAYS) + CYCLE_LENGTH_DAYS) % CYCLE_LENGTH_DAYS;
  const dayIntoSeason = cycleDay % SEASON_LENGTH_DAYS;
  const msIntoDay = ((now % DAY_MS) + DAY_MS) % DAY_MS;
  return (dayIntoSeason + msIntoDay / DAY_MS) / SEASON_LENGTH_DAYS;
}

export function isInSeason(cropKey, season = currentSeason()) {
  return CROPS[cropKey]?.season === season;
}

// Rendement selon l'adéquation saison/culture :
//  - 100% dans sa propre saison
//  - 10% dans sa "pire" saison (la saison juste avant la sienne dans le
//    cycle — un choix qui garantit que chaque culture a une pire saison
//    différente des 3 autres)
//  - 20% dans les deux saisons restantes
export function cropYieldSeasonMult(cropKey, season = currentSeason()) {
  const crop = CROPS[cropKey];
  if (!crop) return 1;
  const ownIdx = SEASONS.indexOf(crop.season);
  const curIdx = SEASONS.indexOf(season);
  if (curIdx === ownIdx) return 1;
  const worstIdx = (ownIdx - 1 + SEASONS.length) % SEASONS.length;
  if (curIdx === worstIdx) return 0.10;
  return 0.20;
}

export function totalStock(state) {
  return CROP_KEYS.reduce((sum, k) => sum + (state.stocks?.[k] || 0), 0);
}

export function cropSellPrice(cropKey, season = currentSeason()) {
  const crop = CROPS[cropKey] || CROPS.ble;
  return isInSeason(cropKey, season) ? crop.price : crop.price * OFF_SEASON_PRICE_PENALTY;
}

// Valeur totale du silo si tout était vendu maintenant, aux prix du jour.
export function stockValue(state, season = currentSeason()) {
  return CROP_KEYS.reduce((sum, k) => sum + (state.stocks?.[k] || 0) * cropSellPrice(k, season), 0);
}
...
export const UPGRADE_DEFS = {
  ...
  sacOuvrier: { max: 3, baseCost: 200, name: 'Sacs multi-parcelles' },
  sacSemeur: { max: 3, baseCost: 200, name: 'Pochette de semences' },
};

// --- Multi-plot bags (ouvrier "sacOuvrier" / semeur "sacSemeur") ---
// Rather than pushing the automation interval below a readable 2s floor, or
// piling on more workers once a 16x16 grid makes that visually noisy, each
// action clears/sows several adjacent plots at once. Level 0 = 1 plot per
// action (the baseline, no purchase needed); each level bought adds one
// more plot per action, up to 4 total.
export const BAG_MIN_GEN_LEVEL2 = 4; // unlocks buying up to "2 plots per action"
export const BAG_MIN_GEN_LEVEL3PLUS = 6; // unlocks levels 3 and 4 (3-4 plots per action)

export function bagSize(state, key) {
  return (state.upgrades[key]?.level || 0) + 1;
}

export function bagRequiredGen(nextLevelCount) {
  // nextLevelCount is the "Niveau Sac" being bought (2, 3 or 4 plots/action).
  return nextLevelCount <= 2 ? BAG_MIN_GEN_LEVEL2 : BAG_MIN_GEN_LEVEL3PLUS;
}

export function bagUpgradeCost(key, state) {
  const def = UPGRADE_DEFS[key];
  const u = state.upgrades[key];
  const nextLevelCount = u.level + 2; // level 0 -> buying "niveau sac 2", etc.
  const employees = key === 'sacOuvrier' ? (state.upgrades.ouvrier.count || 1) : (state.upgrades.semeur.count || 1);
  return Math.round(def.baseCost * Math.pow(employees, 1.5) * Math.pow(nextLevelCount, 2.2) * genCostMult(state));
}
...
export function normalizeState(state) {
  ...
  const stocks = { ble: 0, orge: 0, pdt: 0, seigle: 0, ...(state.stocks || {}) };
  // Migration : les parties sauvegardées avant le système multi-cultures
  // avaient un simple nombre `wheat` — on le verse dans le stock de blé.
  if (!state.stocks && typeof state.wheat === 'number' && state.wheat > 0) {
    stocks.ble += state.wheat;
  }
  return {
    ...state,
    wheat: undefined, // remplacé par `stocks` — nettoyé des sauvegardes migrées
    stocks,
    selectedCrop: CROPS[state.selectedCrop] ? state.selectedCrop : 'ble',
...
export function growTimeSeconds(state, cropKey = state.selectedCrop) {
  const crop = CROPS[cropKey] || CROPS.ble;
  const base = Math.max(3, (12 - state.upgrades.irrigation.level * 1.5) / genPowerMult(state));
  const seasonPenalty = isInSeason(cropKey) ? 1 : OFF_SEASON_GROWTH_PENALTY;
  return base * crop.growMult * seasonPenalty;
}

export function yieldAmount(state, cropKey = state.selectedCrop) {
  const crop = CROPS[cropKey] || CROPS.ble;
  const seasonMult = cropYieldSeasonMult(cropKey);
  return Math.round((3 + state.upgrades.graines.level * 2) * genPowerMult(state) * crop.yieldMult * seasonMult);
}
...
export function currentFillBonus(state) {
  ...
  if (cap <= 0) return 0;
  return fillBonusPct(totalStock(state) / cap);
}
...
// --- Field sprite scaling ---
// The wheat sprites are drawn at a native 64x64px. Rather than letting CSS
// stretch cells to arbitrary fractional pixel sizes as the grid grows (which
// turns pixel art to mush), pick the largest of a few clean, simple-fraction
// scales (1, 3/4, 1/2, 3/8) that keeps the whole grid within a comfortable
// width — the same set of steps at 64/48/32/24px per cell that keeps
// image-rendering:pixelated crisp at every farm size up to 16x16.
export const SPRITE_BASE_SIZE = 64;
export const SPRITE_SCALE_STEPS = [1, 0.75, 0.5, 0.375];
export const FIELD_TARGET_WIDTH = 450;

export function spriteScaleForGridSize(size) {
  for (const s of SPRITE_SCALE_STEPS) {
    if (size * SPRITE_BASE_SIZE * s <= FIELD_TARGET_WIDTH) return s;
  }
  return SPRITE_SCALE_STEPS[SPRITE_SCALE_STEPS.length - 1];
}

export function fieldCellSize(state) {
  const size = Math.max(state.farmCols, state.farmRows);
  return Math.round(SPRITE_BASE_SIZE * spriteScaleForGridSize(size));
}
...
export const MAX_FARM_SIZE = 16; // hard ceiling on any farm dimension
...
export function computeResaleValue(state) {
  ...
  return Math.round(plotsValue + techsValue + stockValue(state));
}
...
export function choiceTierCount(state) {
  const genCap = Math.min(BIGGER_TIER_CAP, Math.max(2, state.generation));
  const currentSize = Math.max(state.farmCols, state.farmRows);
  const roomLeft = MAX_FARM_SIZE - currentSize; // largest tier value that still fits under the ceiling
  const sizeCap = Math.max(1, roomLeft + 1); // +1 since tier 0 ("same size") always counts as an option
  return Math.min(genCap, sizeCap);
}
